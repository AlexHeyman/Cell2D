...
public class Transform {   
     ...
     * @throws ArrayIndexOutOfBoundsException if sourceOffset + numberOfPoints * 2 is greater than source.length or the same operation on the destination array   
     */   
...
//        
    ...
    public static Transform createRotateTransform(float angle) {   
        return new Transform((float)Math.cos(angle), -(float)Math.sin(angle), 0, (float)Math.sin(angle), (float)Math.cos(angle), 0);   
    }   
