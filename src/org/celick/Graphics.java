...
import org.celick.geom.Rectangle;
import org.celick.geom.Shape;
import org.celick.geom.ShapeRenderer;
import org.celick.opengl.TextureImpl;
import org.celick.opengl.renderer.LineStripRenderer;
import org.celick.opengl.renderer.Renderer;
import org.celick.opengl.renderer.SGL;
...
public class Graphics {
	...
	public void drawArc(float x1, float y1, float width, float height,
		...
		for (int a = (int) start; a < (int) (end + step); a += step) {
			...
			float x = (float) (cx + (Math.cos(Math.toRadians(ang)) * width / 2.0f));
			float y = (float) (cy + (Math.sin(Math.toRadians(ang)) * height / 2.0f));
	...
	public void fillArc(float x1, float y1, float width, float height,
		...
		if (antialias) {
			...
			for (int a = (int) start; a < (int) (end + step); a += step) {
				...
				float x = (float) (cx + (Math.cos(Math.toRadians(ang + 10))
						* width / 2.0f));
				float y = (float) (cy + (Math.sin(Math.toRadians(ang + 10))
						* height / 2.0f));
